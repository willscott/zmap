...
static udp_src_payload_template_t *udp_template = NULL;
...
int udp_src_global_initialize(struct state_conf *conf) {
	...
	if (strcmp(args, "template-fields") == 0) {
		...
		for (uint32_t i = 0; i < udp_num_template_field_types; ++i) {
			fprintf(stderr, "%s: %s\n",
				udp_src_payload_template_fields[i].name,
				udp_src_payload_template_fields[i].desc);
		}
		fprintf(stderr, "%s\n" ,"");
		unlock_file(stderr);
		exit(0);
	...
	*c++ = 0;
	...
	if (strcmp(args, "text") == 0) {
		...
		if (!inp) {
			free(args);
			free(udp_send_msg);
			log_fatal("udp_src", "could not open UDP data file '%s'\n", c);
			exit(1);
		}
		free(udp_send_msg);
		...
		if (strcmp(args, "template") == 0) {
			...
			udp_template = udp_src_template_load(udp_send_msg, udp_send_msg_len);
		}
		...
		for (i=0; i < udp_send_msg_len; i++) {
			if (sscanf(c + (i*2), "%2x", &n) != 1) {
				free(args);
				free(udp_send_msg);
				log_fatal("udp_src", "non-hex character: '%c'", c[i*2]);
				exit(1);
			}
	...
	if (udp_send_msg_len > MAX_UDP_PAYLOAD_LEN) {
		log_warn("udp_src", "warning: reducing UDP payload to %d "
		...
		__attribute__((unused)) struct state_recv *zrecv)
{
	...
	if (udp_template) {
		udp_src_template_free(udp_template);
		udp_template = NULL;
	}
...
int udp_src_init_perthread(void* buf, macaddr_t *src,
		...
		uint32_t *validation, int probe_num, void *arg)
{
	...
	if (udp_send_substitutions) {
		...
		payload_len = udp_src_template_build(udp_template, payload, MAX_UDP_PAYLOAD_LEN, ip_header, udp_header, aes);
...
void udp_src_process_packet(const u_char *packet, UNUSED uint32_t len, fieldset_t *fs,
...
{
	return udp_src_do_validate_packet(ip_hdr, len, src_ip, validation, num_ports);
}
...
{
	udp_src_payload_field_t *c;

	t->fcount++;
	t->fields = xrealloc(t->fields, sizeof(udp_src_payload_field_t) * t->fcount);
	if (! t->fields) {
		exit(1);
	}

	t->fields[t->fcount - 1] = xmalloc(sizeof(udp_src_payload_field_t));
...
int udp_src_random_bytes(char *dst, int len, const unsigned char *charset,
...
int udp_src_template_build(udp_src_payload_template_t *t, char *out, unsigned int len,
...
int udp_src_template_field_lookup(char *vname, udp_src_payload_field_t *c)
{
	...
	unsigned int f;
	unsigned int olen = 0;
	unsigned int fcount = sizeof(udp_src_payload_template_fields)/sizeof(udp_src_payload_template_fields[0]);
	...
	for (f=0; f<fcount; f++) {
		...
		if (strcmp((char *)vname, udp_src_payload_template_fields[f].name) == 0) {
			c->ftype	= udp_src_payload_template_fields[f].ftype;
			c->length = olen;
			c->data	 = NULL;
			return 1;
		}
...
{
	udp_src_payload_template_t *t = xmalloc(sizeof(udp_src_payload_template_t));
		...
		switch(*p){
			...
			case '}':
				...
				if ( tlen > 0) {
					tmp = xmalloc(tlen);
					memcpy(tmp, s, tlen);
					udp_src_template_add_field(t, UDP_DATA, tlen, tmp);
				}

				tmp = xcalloc(1, p-lbrack);
				memcpy(tmp, lbrack+1, p-lbrack-1);

				if (udp_src_template_field_lookup(tmp, &c)) {
					udp_src_template_add_field(t, c.ftype, c.length, c.data);
...
probe_module_t module_udp_src = {
	...
	.pcap_snaplen = 1500,
	.port_args = 1,
	.thread_initialize = &udp_src_init_perthread,
	.global_initialize = &udp_src_global_initialize,
	.make_packet = &udp_src_make_packet,
	.print_packet = &udp_src_print_packet,
	.validate_packet = &udp_src_validate_packet,
	.process_packet = &udp_src_process_packet,
	.close = &udp_src_global_cleanup,
	.helptext = "Probe module that sends UDP packets to one host. Packets can "
	            "optionally be templated based on spoofed source. Specify"
