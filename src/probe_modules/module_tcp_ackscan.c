...
// probe module for performing TCP SYNACK scans
...
int ackscan_global_initialize(struct state_conf *state)
{
	num_ports = state->source_port_last - state->source_port_first + 1;
	return EXIT_SUCCESS;
}

int ackscan_init_perthread(void* buf, macaddr_t *src,
...
int ackscan_make_packet(void *buf, ipaddr_n_t src_ip, ipaddr_n_t dst_ip,
...
void ackscan_print_packet(FILE *fp, void* packet)
{
	...
	fprintf(fp, "tcpack { source: %u | dest: %u | seq: %u | checksum: %u }\n",
			ntohs(tcph->th_sport),
			ntohs(tcph->th_dport),
			ntohl(tcph->th_seq),
			ntohl(tcph->th_sum));
...
int ackscan_validate_packet(const struct ip *ip_hdr, uint32_t len,
...
void ackscan_process_packet(const u_char *packet,
		__attribute__((unused)) uint32_t len, fieldset_t *fs, __attribute__((unused)) uint32_t *validation)
{
...
probe_module_t module_tcp_ackscan = {
	...
	.helptext = "Probe module that sends a TCP SYNACK packet to a specific "
		"port. Possible classifications are: rst. A "
		"reset packet properly responding to the probe is considered success.",
