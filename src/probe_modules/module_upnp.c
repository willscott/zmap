...
static const char const *upnp_query = "M-SEARCH * HTTP/1.1\r\n"
		...
		macaddr_t *gw, port_h_t dst_port)
{
	...
	uint16_t len = htons(sizeof(struct iphdr) + sizeof(struct udphdr) + strlen(upnp_query));
	...
	assert(sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr) 
